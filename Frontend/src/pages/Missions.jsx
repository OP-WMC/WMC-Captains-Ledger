...
  ChevronLeft, ChevronRight // Import Chevron icons for pagination
} from 'lucide-react';
import Modal from 'react-modal';
import Loader from '../components/Loader';

// Define the number of missions per page for desktop and mobile
const MISSIONS_PER_PAGE_DESKTOP = 4;
const MISSIONS_PER_PAGE_MOBILE = 3;
...
  // New states for Accept/Decline Mission Modal
...
  // Set the number of missions per page dynamically
  const missionsPerPage = isMobile ? MISSIONS_PER_PAGE_MOBILE : MISSIONS_PER_PAGE_DESKTOP;
...
    const now = new Date();
    if (start < now && !showEditModal) { // Only validate for past date on creation, not edit
...
  const allFilteredMissions = (isAdmin ? missions : userMissions).filter(m =>
    filter === 'all' || m.status === filter
  );

  // Pagination Logic
  const totalPages = Math.ceil(allFilteredMissions.length / missionsPerPage);
  const paginatedMissions = allFilteredMissions.slice(
...
  // Logic for accepting/declining missions
  useEffect(() => {
    if (!isAdmin && user && missions.length > 0) {
      const now = new Date();
...
      const uploadRes = await axios.post('/missions/upload', formData, { withCredentials: true });
...
    fetchMissions(); // Re-fetch missions to update status
  } catch (err) {
    console.error("Decline mission failed:", err); // Keep this for debugging
...
                  className="px-3 py-1 sm:px-4 sm:py-2 rounded-lg font-bold transition-all duration-200 shadow-lg border bg-white hover:bg-blue-500 dark:border-gray-600 dark:bg-gray-500 text-blue-500 dark:text-white dark:hover:bg-gray-700 hover:text-white disabled:bg-white disabled:text-blue-500 disabled:cursor-not-allowed disabled:opacity-40 text-sm"
                >
                  <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5 inline-block mr-1" /> Prev
                </button>
                <span className="text-blue-700 dark:text-white font-semibold text-sm sm:text-base">
...
                  className="px-3 py-1 sm:px-4 sm:py-2 rounded-lg font-bold transition-all duration-200 shadow-lg border bg-white hover:bg-blue-500 dark:border-gray-600 dark:bg-gray-500 text-blue-500 dark:text-white dark:hover:bg-gray-700 hover:text-white disabled:bg-white disabled:text-blue-500 disabled:cursor-not-allowed disabled:opacity-40 text-sm"
                >
                  Next <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 inline-block ml-1" />
                </button>
              </div>
            )}
          </>
...
        {/* Mission Assignment Modal (for users to accept/decline) */}
        <Modal
...
                  <label className="block text-gray-700 dark:text-cyan-200 font-semibold mb-2">Reason for Decline</label>
                  <textarea
                    className="w-full p-2 rounded border border-gray-400 dark:bg-gray-800 dark:text-white"
                    value={declineReason}
...
                    required
                  />
                  <label className="block mt-4 text-gray-700 dark:text-cyan-200 font-semibold mb-2">Attach File (optional)</label>
                  <input
                    type="file"
                    className="w-full text-gray-700 dark:text-cyan-200"
